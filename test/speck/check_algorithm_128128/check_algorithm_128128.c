...
static const uint8_t s_plain_text_stream[16] = { 0x20, 0x6d, 0x61, 0x64,
                                                 0x65, 0x20, 0x69, 0x74,
                                                 0x20, 0x65, 0x71, 0x75,
                                                 0x69, 0x76, 0x61, 0x6c, };
static const uint8_t s_cipher_text_stream[16] = { 0x18, 0x0d, 0x57, 0x5c,
                                                  0xdf, 0xfe, 0x60, 0x78,
                                                  0x65, 0x32, 0x78, 0x79,
                                                  0x51, 0x98, 0x5d, 0xa6, };
...
int encrypt_stream_test(int block_num, const char *test_explain) {
    ...
    if(!plain_text_stream) { r = 1; goto finish; }
    crypted_text_stream = malloc(BLOCK_SIZE * block_num);;
    if(!crypted_text_stream) { r = 1; goto finish; }
    expect_crypted_stream = malloc(BLOCK_SIZE * block_num);;
    if(!expect_crypted_stream) { r = 1; goto finish; }

    ctx = speck_init(SPECK_ENCRYPT_TYPE_128_128, s_key);
    if(!ctx) { r = 1; goto finish; }
...
int decrypt_stream_test(int block_num, const char *test_explain) {
    ...
    if(!decrypted_text_stream) { r = 1; goto finish; }
    expect_decrypted_stream = malloc(BLOCK_SIZE * block_num);;
    if(!expect_decrypted_stream) { r = 1; goto finish; }
...
int main() {
    ...
    // encrypt test
    {
        speck_ctx_t *ctx = speck_init(SPECK_ENCRYPT_TYPE_128_128, s_key);
        if(!ctx) return 1;

        uint64_t crypted_text[2];

        speck_encrypt(ctx, s_plain_text, crypted_text);
        if(!(crypted_text[0] == s_cipher_text[0]  &&
             crypted_text[1] == s_cipher_text[1])) {
    ...
    // decrypt test
    {
        ...
        if(!(decrypted_text[0] == s_plain_text[0]  &&
             decrypted_text[1] == s_plain_text[1])) {
